package com.desen.desenmall.cart.interceptor;


import com.desen.common.constant.AuthServerConstant;
import com.desen.common.constant.CartConstant;
...
public class CartInterceptor implements HandlerInterceptor {
    ...
    public boolean preHandle(HttpServletRequest request, HttpServletResponse response, Object handler) throws Exception {
        ...
        if (StringUtils.isEmpty(userInfoTo.getUserKey())){
            ...
            userInfoTo.setUserKey("desen-" + uuid);
        }
    ...
    public void postHandle(HttpServletRequest request, HttpServletResponse response, Object handler, ModelAndView modelAndView) throws Exception {
        ...
        // 如果没有临时用户 则让浏览器保存临时用户
        if(!userInfoTo.isTempUser()){
            Cookie cookie = new Cookie(CartConstant.TEMP_USER_COOKIE_NAME, userInfoTo.getUserKey());
            // 设置这个cookie作用域 过期时间
            cookie.setDomain("desenmall.com");
            cookie.setMaxAge(CartConstant.TEMP_USER_COOKIE_TIME_OUT);
            response.addCookie(cookie);
        }
        threadLocal.remove();//用完移除，避免内存泄露
    }
}
