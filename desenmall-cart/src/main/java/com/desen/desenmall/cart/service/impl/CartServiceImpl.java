package com.desen.desenmall.cart.service.impl;

import com.alibaba.fastjson.JSON;

import com.alibaba.fastjson.TypeReference;
import com.desen.common.utils.R;
import com.desen.desenmall.cart.feign.ProductFeignService;
import com.desen.desenmall.cart.interceptor.CartInterceptor;
import com.desen.desenmall.cart.service.CartService;
import com.desen.desenmall.cart.vo.Cart;
import com.desen.desenmall.cart.vo.CartItem;
import com.desen.desenmall.cart.vo.SkuInfoVo;
import com.desen.desenmall.cart.vo.UserInfoTo;
...
public class CartServiceImpl implements CartService {
    ...
    private final String CART_PREFIX = "desen:cart:";

    @Override
    public CartItem addToCart(Long skuId, Integer num) throws ExecutionException, InterruptedException {
        return addToCart(skuId,num,null);
    }


    public CartItem addToCart(Long skuId, Integer num, CartItem cartItemTmp) throws ExecutionException, InterruptedException {
        BoundHashOperations<String, Object, Object> cartOps = getCartOps();
        String res = (String) cartOps.get(skuId.toString());
        if (StringUtils.isEmpty(res)) {
            //针对临时购物车的合并
            if(cartItemTmp != null){
                cartOps.put(skuId.toString(), JSON.toJSONString(cartItemTmp));
                return cartItemTmp;
            }
    ...
    public Cart getCart() throws ExecutionException, InterruptedException {
        ...
        if (userInfoTo.getUserId() != null) {
            ...
            if (tempItem != null) {
                // 1.2 临时购物车有数据 则进行合并
                log.info("[" + userInfoTo.getUsername() + "] 的购物车已合并");
                for (CartItem cartItem : tempItem) {
                    addToCart(cartItem.getSkuId(), cartItem.getCount(), cartItem);
                }
                // 1.3 清空临时购物车
    ...
    private BoundHashOperations<String, Object, Object> getCartOps() {
        ...
        if (userInfoTo.getUserId() != null) {
            log.debug("用户 [" + userInfoTo.getUsername() + "] 正在操作购物车");
            // 已登录的用户购物车的标识
            cartKey += userInfoTo.getUserId();
        } else {
            log.debug("临时用户 [" + userInfoTo.getUserKey() + "] 正在操作购物车");
            // 未登录的用户购物车的标识
